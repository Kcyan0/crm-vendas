        ...
        <div className="max-w-7xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-sec mt-1 font-medium">
            ...
            <div className="bg-app border border-green-200 rounded-2xl p-4 flex gap-3 text-green-800">
            ...
            {/* Criar Novo Projeto */}
            <div className="bg-surface rounded-3xl border border-str shadow-sm overflow-hidden">
                ...
                <div className="p-6 bg-app/50">
                    <form onSubmit={handleCreateProject} className="flex flex-col md:flex-row gap-4 items-end">
                        <div className="flex-1 w-full relative">
                            <label className="block text-xs font-bold text-sec uppercase tracking-wider mb-2 ml-1">
                                Nome do Projeto
                            </label>
                            <input
                                type="text"
                                ...
                                className="w-full bg-surface border border-str text-white text-sm rounded-xl focus:ring-2 focus:ring-green-500 focus:border-green-500 block p-3.5 transition-all shadow-sm"
                                required
                            />
                        </div>
                        <div className="flex-1 w-full relative">
            ...
            <div className="space-y-6">
                <div>
                    <h2 className="text-xl font-bold text-white flex items-center gap-2">
                        Seus Projetos
                        <span className="bg-surface text-[#AAAAAA] text-xs py-1 px-2.5 rounded-full font-bold">
                            {projetos.length}
                        </span>
                    </h2>
                    <p className="text-sm text-sec mt-1">
                    ...
                    {projetos.map((projeto) => {
                        ...
                        return (
                            <div
                                ...
                                className={`bg-surface rounded-3xl border ${isSelected ? 'border-green-500 ring-4 ring-green-50' : 'border-str hover:border-slate-300'} p-6 shadow-sm transition-all relative overflow-visible flex flex-col h-full`}
                            >
                                <div className="absolute top-4 right-4 flex items-center gap-2">
                                    {isSelected && (
                                        <div className="bg-surface text-green-700 p-1.5 rounded-full">
                                    ...
                                    <div className="relative">
                                        <button
                                            ...
                                            className="p-1.5 text-sec hover:bg-surface rounded-full transition-colors"
                                        >
                                            <MoreVertical size={20} />
                                        </button>

                                        {openMenuId === projeto.id_projeto && (
                                            <div className="absolute right-0 mt-2 w-32 bg-surface rounded-xl shadow-lg shadow-black/10 border border-[#222222] z-10 py-1 overflow-hidden">
                                                <button
                                                    onClick={() => handleOpenEdit(projeto)}
                                                    className="w-full text-left px-4 py-2 text-sm text-white hover:bg-app transition-colors font-medium"
                                                >
                                                    Editar
                                                </button>
                                                <button
                                ...
                                <div className="mb-4 pr-16 mt-2">
                                    ...
                                    <p className="text-sm text-sec line-clamp-2 mt-1 min-h-[40px]">
                                ...
                                <div className="mt-auto pt-6 border-t border-[#222222]">
                                    {isSelected ? (
                                        <button
                                            disabled
                                            className="w-full bg-app text-sec font-bold py-3 px-4 rounded-xl flex justify-center items-center gap-2 cursor-default border border-[#222222]"
                                        >
            ...
            {isEditModalOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
                    <div className="bg-surface rounded-3xl w-full max-w-md shadow-2xl overflow-hidden border border-str animate-in zoom-in-95 duration-200">
                        <div className="px-6 py-4 border-b border-[#222222] flex justify-between items-center bg-app/50">
                            <h3 className="font-bold text-white text-lg">Editar Projeto</h3>
                            <button
                                onClick={() => setIsEditModalOpen(false)}
                                className="text-sec hover:text-[#AAAAAA] p-1 rounded-full hover:bg-surface-3 transition-colors"
                            >
                                &times;
                            </button>
                        </div>
                            ...
                            <form onSubmit={handleEditSubmit} className="space-y-4">
                                <div>
                                    ...
                                    <input
                                        ...
                                        className="w-full bg-app border border-str text-white text-sm rounded-xl focus:ring-2 focus:ring-green-500 block p-3.5 transition-all outline-none"
                                        required
                                    />
                                </div>
                                <div>
                                ...
                                <div className="pt-4 flex gap-3">
                                    <button
                                        type="button"
                                        onClick={() => setIsEditModalOpen(false)}
                                        className="flex-1 px-4 py-3 border border-str text-[#AAAAAA] rounded-xl hover:bg-app font-bold transition-all text-sm"
                                    >
                                        Cancelar
                                    </button>
                                    <button
