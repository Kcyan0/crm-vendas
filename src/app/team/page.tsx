        ...
        <div className="pt-4 h-full flex flex-col">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end mb-8 gap-4">
                <div>
                    ...
                    <p className="text-sec mt-1">Configure permissões, comissionamentos e salários.</p>
                </div>
            ...
            <div className="glass-panel overflow-hidden bg-surface flex flex-col flex-1 min-h-0">
                <div className="overflow-x-auto overflow-y-auto flex-1">
                    <table className="w-full text-left border-collapse">
                        <thead className="sticky top-0 z-10">
                            <tr className="border-b border-str bg-app">
                                <th className="p-4 text-xs font-semibold text-sec tracking-wider uppercase">Nome / Cargo</th>
                                <th className="p-4 text-xs font-semibold text-sec tracking-wider uppercase">Status</th>
                                <th className="p-4 text-xs font-semibold text-sec tracking-wider uppercase">Salário Fixo</th>
                                <th className="p-4 text-xs font-semibold text-sec tracking-wider uppercase">Comissão SDR</th>
                                <th className="p-4 text-xs font-semibold text-sec tracking-wider uppercase">Comissão Closer</th>
                                <th className="p-4 text-xs font-semibold text-sec tracking-wider uppercase text-right">Ações</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-100">
                            {team.map((member) => (
                                <tr key={member.id_usuario} className="hover:bg-app transition-colors">
                                    <td className="p-4">
                                        <div className="flex items-center gap-3">
                                            <div className="w-10 h-10 rounded-full bg-surface flex items-center justify-center text-white font-bold border border-str">
                                    ...
                                    <td className="p-4">
                                        <div className="flex items-center gap-2 text-white font-bold">
                                            <CreditCard size={14} className="text-sec" />
                            ...
                            {team.length === 0 && (
                                <tr>
                                    <td colSpan={6} className="p-8 text-center text-sec">
                                        Carregando membros...
                                    </td>
                                </tr>
                            )}
            ...
            {isModalOpen && (
                <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center bg-slate-900/40 backdrop-blur-sm p-0 md:p-4">
                    <div className="glass-panel w-full max-w-lg p-4 md:p-6 relative bg-surface max-h-[90vh] overflow-y-auto rounded-t-2xl md:rounded-2xl">
                        ...
                        <form onSubmit={handleSaveUser} className="space-y-4">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <select className="w-full bg-surface border border-str" value={formData.tipo} onChange={(e) => setFormData({ ...formData, tipo: e.target.value })}>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4 pt-4 border-t border-[#222222]">
                                <div>
                                    ...
                                    <input type="number" step="0.01" className="w-full bg-surface" value={formData.salario} onChange={(e) => setFormData({ ...formData, salario: e.target.value })} />
                                </div>
                                <div>
                                    <label className="block text-sm font-medium text-white mb-1">% SDR</label>
                                    <input type="number" step="0.1" className="w-full bg-surface" value={formData.pctSdr} onChange={(e) => setFormData({ ...formData, pctSdr: e.target.value })} />
                                </div>
                                <div>
                                    <label className="block text-sm font-medium text-white mb-1">% Closer</label>
                                    <input type="number" step="0.1" className="w-full bg-surface" value={formData.pctCloser} onChange={(e) => setFormData({ ...formData, pctCloser: e.target.value })} />
                                </div>
                            </div>
                            ...
                            <div className="flex justify-between items-center mt-8 pt-4 border-t border-[#222222] flex-col-reverse md:flex-row gap-4">
                                ...
                                <div className="flex gap-3 w-full md:w-auto">
                                    <button type="button" onClick={() => setIsModalOpen(false)} className="flex-1 md:flex-none px-4 py-2 text-sec hover:text-white transition-colors">Cancelar</button>
