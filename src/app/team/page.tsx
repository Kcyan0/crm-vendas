    ...
    return (
        <div className="pt-4 h-full flex flex-col">
            <div className="flex justify-between items-end mb-8">
                <div>
                    <h2 className="text-3xl font-bold text-slate-800 tracking-tight">Gestão do Time</h2>
                    <p className="text-slate-500 mt-1">Configure permissões, comissionamentos e salários.</p>
                </div>
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="border-b border-slate-200 bg-slate-50">
                                <th className="p-4 text-xs font-semibold text-slate-500 tracking-wider uppercase">Nome / Cargo</th>
                                <th className="p-4 text-xs font-semibold text-slate-500 tracking-wider uppercase">Status</th>
                                <th className="p-4 text-xs font-semibold text-slate-500 tracking-wider uppercase">Salário Fixo</th>
                                <th className="p-4 text-xs font-semibold text-slate-500 tracking-wider uppercase">Comissão SDR</th>
                                <th className="p-4 text-xs font-semibold text-slate-500 tracking-wider uppercase">Comissão Closer</th>
                                <th className="p-4 text-xs font-semibold text-slate-500 tracking-wider uppercase text-right">Ações</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-100">
                            {team.map((member) => (
                                <tr key={member.id_usuario} className="hover:bg-slate-50 transition-colors">
                                    <td className="p-4">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                <p className="font-semibold text-slate-800">{member.nome}</p>
            ...
            {isModalOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4">
                    <div className="glass-panel w-full max-w-lg p-6 relative bg-white">
                        ...
                        <form onSubmit={handleSaveUser} className="space-y-4">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <select className="w-full bg-white border border-slate-200" value={formData.tipo} onChange={(e) => setFormData({ ...formData, tipo: e.target.value })}>
                            ...
                            <div className="grid grid-cols-3 gap-4 mt-4 pt-4 border-t border-slate-100">
                                <div>
                                    ...
                                    <input type="number" step="0.01" className="w-full bg-white" value={formData.salario} onChange={(e) => setFormData({ ...formData, salario: e.target.value })} />
                                </div>
                                <div>
                                    <label className="block text-sm font-medium text-slate-700 mb-1">% SDR</label>
                                    <input type="number" step="0.1" className="w-full bg-white" value={formData.pctSdr} onChange={(e) => setFormData({ ...formData, pctSdr: e.target.value })} />
                                </div>
                                <div>
                                    <label className="block text-sm font-medium text-slate-700 mb-1">% Closer</label>
                                    <input type="number" step="0.1" className="w-full bg-white" value={formData.pctCloser} onChange={(e) => setFormData({ ...formData, pctCloser: e.target.value })} />
                                </div>
                            </div>
